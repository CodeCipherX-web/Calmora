...
(function() {
  ...
  function getApiBaseUrl() {
    ...
    const apiPort = '3001'; // Updated to match server port

    // If accessing via file:// protocol, default to localhost:3001
    if (protocol === 'file:') {
      return `http://localhost:${apiPort}`;
    }

    // If using Live Server (usually port 5500), use localhost:3001 for API
  ...
  async function apiRequest(endpoint, options = {}) {
    ...
    try {
      ...
      if (error.name === 'TypeError' && (error.message.includes('fetch') || error.message.includes('Failed to fetch'))) {
        const networkError = new Error('Network error: Unable to connect to server. Please make sure the server is running on http://localhost:3001');
        networkError.type = 'network';
        throw networkError;
      }
      
      // Handle connection refused or other network issues
